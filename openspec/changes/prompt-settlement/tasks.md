...
- [x] 2.1 turn action 在 accepted 后具有非空 `promptIds` 与逐 owner 独立 `runLeases`；公共 action 单 owner、internal inherit 可多 owner。create/restore 只准备候选或私有 `{inherit: owners}` lineage，不写 tracker。`ActionStore.assertCanEnqueue`（或等价纯 preflight）与 disposing、coalesce、action-id/候选-id检查全部在tracker admission前完成；fresh default随后同步 `admit({sessionEpoch})` + `acquire("run")`（no-op persist/Date.now下fresh identity保证acquire不可失败）再enqueue。inherit先验证全部owners仍settling，再逐owner acquire；任一失败释放本action本次已取得的sibling leases，绝不cancel/释放parent既有lease，整action不入队。成功enqueue后才隔离地触发一次settlement-admission callback。session/extension/handled与session command无action settlement字段；成功non-turn仍恰调用一次callback为`{supported:true}`且无`promptId`，失败不虚报accepted id；tracker emit接session event，persist暂no-op
- [x] 2.2 preparing 禁止重复 acquire；`"all"` batching 只把本批 turn actions 的 `promptIds` 去重写入 `_lastRunPromptIds`/`_currentRunOwners`（组2新增owner snapshot，组7只消费它记录message ids）。完成、terminal provider error/aborted、pump failure 或实际从队列移除通过单一idempotent helper仅一次消费action已存leases并清空引用：本批最终stopReason为error时先逐owner `recordFailure("run_error")`、为aborted时先`requestCancel`，每批清空signal避免stale；default queue clear/mutate delete先`requestCancel`再release；inherit非-abort取消只release；unknown/terminal/busy inherit owner使整action放弃。pre-admission无record/lease；recovery snapshot不含`promptIds`或live `runLeases`，普通restore turn生成fresh id；primary为heartbeat/RLM notice则重派生background exclusion且prefix不得改变普通primary identity。保证无0-lease accepted action或永久settling。主动in-flight `requestAbort` ownership清理与dispose released fence仍属组6；组2 outcome的`finalMessageIds`固定为空，组7才填充
...
- [x] 2.4 行为测试（Seam 2，faux provider，retry关闭）：单run completed且event/query/wait同一对象、`finalMessageIds=[]`；terminal provider error与pump throw均先recordFailure再一次release为failed/run_error，provider aborted为cancelled且`"all"`全部owners一致，前一failed signal不污染后一success；两个排队prompt独立；`"all"` batching全部owners各持action run lease，mid-run只读inspection证明`_lastRunPromptIds`/`_currentRunOwners`恰为本batch去重owners，且共享run结束前均不结算；同步tool返回前不结算；run中steer与followUp各拿独立identity；session/extension/handled成功non-turn均回调一次`{supported:true}`无id、返回undefined且不占候选，失败仍reject且不虚报id；accepted queued default经clearQueue/mutate delete→cancelled且二次clear/finally零额外event；私有inherit seam覆盖真实第二次acquire失败后的sibling rollback不碰parent lease、unknown/terminal/busy整action不入队、实际非-abort removal只release不cancel；coalesce/disposing/重复action/lifecycle/预分配id均在tracker admission前reject且无新event/identity；并发callbacks不串id、occupied agentMessageId拒绝且callback throw不影响action；recovery snapshot无promptIds/runLeases，普通restore生成fresh id、background primary仍排除、background prefix不误分类普通primary；heartbeat已pending时main outcome不阻塞；AgentSession直接订阅可见而in-process main/watcher与daemon broadcast不可见。测试通过callback、query/event与必要的只读action-store inspection观察，不依赖private persist count。retry/compaction、主动in-flight requestAbort ownership清理、dispose released fence、message id记录、ledger与外部modes/wire接线分别留后续组
