...
- [x] 6.3 failure/teardown 与 sibling compatibility 证据：异步 factory 原始 reject 经真实 caller 按 identity reject，恢复 editor/focus并推进后继；真实 `resetExtensionUI` 使 visible 与 queued custom 各以 `AbortError` 恰好 reject、queued factory 从未调用；constructing custom 被 reset 后迟到 resolve 的组件不挂载且 dispose 恰好一次。overlay custom 在 app-kind blocker 存在时仍立即调用 factory/showOverlay，static/dynamic options、`onHandle`、done 值与 dispose 保持既有行为且不触碰 arbiter surface。Round 1 depth closure 另以 Seam 1 覆盖 callback 内 editor replacement 的动态重读/非递归多轮收敛、8 轮 fail-closed/no microtask growth/terminal escape，mounted cleanup 各阶段重入 `disposeAll()` 时在 nested call 返回前完成一次 dispose→clear→focus-null→render，以及 cleanup/onEvict callback 内嵌套结算 queued 请求后新请求仍延迟到外层 cleanup 完成后的 handoff；真实 focusable editor setter 经 public replacement 验证最终 current/surface/focus identity 配对。Phase 6.2 另覆盖 stale-clear / restore / failClosed 的 replace/focus/render host throw：错误被隔离、handoff 微任务不得 unhandled、后继仍构造或 fail-closed 保持 busy，以及 mounted-cleanup render throw 与 clear/focus 同矩阵。固定 seam 为 `packages/coding-agent/test/interactive-mode-extension-custom.test.ts` 与 `packages/coding-agent/test/dialog-arbiter.test.ts`；从 `packages/coding-agent` 运行 `npx tsx ../../node_modules/vitest/dist/cli.js --run test/interactive-mode-extension-custom.test.ts test/dialog-arbiter.test.ts`，期望退出 0 且上述 FIFO、快照/实例配对、error/reset/late cleanup、bounded restore/terminal reentry、handoff host-error containment、overlay assertions 全部通过。
...
- [x] 9.1 将 `handleShareCommand` 的 `BorderedLoader` 展示/恢复改写为 `arbiter.present`（kind: placeholder，`cancel: () => undefined`）：每次 invocation 先生成独立 temp path，再由 export/`gh`/cleanup 全程使用；完成路径经 `settle`，取消路径经组件 `onAbort` 映射到 `show` 内的 `done`（无外部 AbortSignal）并保持杀死活动 `gh` 子进程与“Share cancelled”反馈；临时文件删除收敛为带 once guard 的幂等清理函数。已挂载请求仅由组件 dispose 路径调用该函数，未挂载请求仅由 `onEvict` 调用，两个 ownership 路径互斥；export 失败也清理本 invocation 可能留下的部分文件。命令对称观察 dialog result 与 child close/error：用户取消或 session dispose 先于子进程完成时立即停止后续 share 状态/错误输出，process-first 时完成结果不被同 turn 的后到取消覆盖，迟到 close/error 不得二次结算或二次清理；export pending 时 terminal teardown 完成后不启动子进程或写 UI。
- [x] 9.2 新增 `packages/coding-agent/test/interactive-mode-share-loader.test.ts`，仅用系统边界 mock/临时目录，不调用真实 `gh`、provider、API key 或付费 token。场景证据：(a) 空闲成功：auth 成功、export 完成、mock `gh gist create` 成功，loader 同步展示，完成后 external settle、组件 dispose、临时文件删除、editor/focus 恢复与 share URL 各恰好一次；(b) app-kind blocker 可见时 share 成功先完成，placeholder 展示前 settle，断言 loader 从未构造/挂载、`onEvict` 与临时文件删除各一次，blocker 不被替换；(c) blocker 可见且 `gh` pending 时重复 `disposeAll()`，断言 queued loader 从未构造/挂载、所属 process kill、cancel/onEvict/临时文件删除各一次，迟到 close 不产生状态/错误、第二次清理或 UI；(d) loader 已 visible、`gh` pending 时重复 `disposeAll()`，断言 process kill、组件 dispose、删除各一次，editor 不恢复且随后 `ui.stop()`；迟到 close 无状态/错误/第二次清理/任何 UI；(e) loader 可见时触发真实组件 cancel，断言 `gh` kill、`done` 结算、组件 dispose、删除、取消反馈与 editor/focus 恢复各一次，随后 close 不产生第二个 outcome；(f) `exportToHtml` 写入部分 temp file 后 reject，断言删除一次、loader/request 从未出现、既有 `Failed to export session: …` 一次且 arbiter UI 未写；(g) mock spawn 同步 throw 与 process nonzero 两条失败输入，均断言 settle、mounted dispose、删除、editor/focus 恢复和既有 `Failed to create gist: …` 各一次，无第二 outcome；(h) auth preflight 失败且存在同名 foreign file，断言不 export、不 present、不删文件，只输出既有 auth error；(i) 两次并发 share 获得不同 temp path，第二条先完成/清理后第一条的文件仍存在且第一条随后独立成功；一条取消时同样不 kill/delete/settle 另一条；(j) process close 先到、用户 cancel 在同一 turn 后到，仍以 process completion 为 outcome；反向顺序保持 cancel outcome；(k) child 异步 `error` settle/cleanup/restore/单错误反馈，无 unhandled `error`；(l) export pending 时重复 `disposeAll()` 后 export resolve/reject，独立 path 清理一次、不 spawn、不写任何 status/error/UI；(m) idle mount 的 replace/focus/render 任一 host callback 同步抛错，request rejection 先于 child start，断言 cleanup 一次、不 spawn、不报告 gist outcome；(n) user cancel 的 process `kill()` 同步触发 close 时仍由已进入的 cancel edge 胜出，只输出取消反馈，close 不得覆盖。固定命令：从 `packages/coding-agent` 运行 `npx tsx ../../node_modules/vitest/dist/cli.js --run test/interactive-mode-share-loader.test.ts test/dialog-arbiter.test.ts`，期望退出 0。

Suggested fixture level: compact - 含资源清理路径迁移（现状清理只在展示后路径上执行），需泄漏场景验证
Effective fixture level: expanded - `/share` 是共享 TUI command entrypoint，本 PR 同时转移 `editorContainer`/focus、取消/子进程竞态与临时文件删除 ownership，命中 mandatory entrypoint、temp/delete、cancellation/shared-state trigger 和项目 TUI/session teardown domain trigger。Repair intensity: high - 临时文件删除、外部子进程与 terminal teardown 必须以 scoped invariant 和失败矩阵闭合。
Selected risk packs: Public API / CLI / script entry（保持 `/share` auth/export/gist/status 契约）、File IO / path safety / overwrite（只删除本次 invocation 的独立导出路径，所有已创建/部分创建路径闭合）、Concurrency / shared state / ordering（process close 与 done/settle/dispose 首终止边）、Legacy compatibility / examples（成功、错误与用户取消反馈保持）、Error handling / rollback / partial outputs（export/spawn/close/teardown 清理）、TUI focus/render lifecycle（共享 surface ownership）、Session/extension teardown lifecycle（dispose 后无迟到 UI）。Config、schema、auth/permissions/secrets、resource limits/discovery、release/packaging、documentation 不选：不改变配置/格式、既有 `gh auth status` 与 private gist 参数、等待策略、依赖或外部文档。
Invariant Matrix (group 9):
- Governing invariant: export 开始后，本次 share 所属临时文件只由该请求的幂等清理闭包删除；第一个 process completion、用户取消或 session dispose 决定 terminal outcome，后到事件不再触碰 UI/资源。
- Source of truth: 单个 `handleShareCommand` invocation 的 temp path、cleanup guard、arbiter request settle-once 状态和活动 child process identity。
- Surfaces: producer=`exportToHtml`；preflight=`gh auth status`；entrypoint=`/share`/`handleShareCommand`；consumer=`BorderedLoader`/share status；failure/rollback=`export reject`、spawn throw/nonzero、user abort、queued/visible `disposeAll`、late close；evidence=`interactive-mode-share-loader.test.ts`。
- Regression rows: 正常成功 -> mounted dispose 删除一次并恢复 editor；展示前 process 完成 -> onEvict 删除一次且 loader 不闪现；queued/visible teardown 或用户取消与 close 竞争 -> 第一条终止边胜出、kill（用户取消/活动 teardown）、清理一次、无迟到 UI；auth 失败 -> 不 export、不创建请求、不删除无所属文件。
Boundary checklist (group 9): `handleShareCommand` 是唯一 producer/consumer integration；`BorderedLoader.dispose` 与 request `onEvict` 是互斥 cleanup owners；`DialogArbiter` 不获得文件路径知识；其他 temp-file/export 调用点和 gist 内容/权限流程保持不变。
Minimal mergeable slice: atomic - 单调用点 + 配套测试，独立合并保绿

## 10. 收口验证

- [x] 10.1 新增 source negative oracle（非 lint）：读取 `interactive-mode.ts` 原始源码，只以 `editorContainer.clear(` 与 `editorContainer.addChild(` 两个精确模式枚举每个命中；通过函数/闭包范围定位命中的 innermost owner，并逐项断言 owner 属于显式白名单：constructor 的 editor 初始挂载、constructor 内注入给 arbiter 的 `replaceEditorSurface` 闭包、`setCustomEditorComponent`。失败消息必须打印实际模式、owner 与 1-based 行号；不使用总命中数或固定行号断言。该检查只证明 owner，`setCustomEditorComponent` 空闲/忙 guard 继续由任务 2.2 行为测试负责。Oracle 自校验：临时在一个非白名单 method 中增加 `this.editorContainer.clear()`，运行本测试必须以该 method/行号变红，再移除并恢复源码 byte-for-byte。
- [x] 10.2 新增 `packages/coding-agent/test/interactive-mode-dialog-arbiter-closeout.test.ts`（或等价单一 closeout suite），仅增/改测试、不改产品源码，覆盖四条真实入口链：(a) 经真实 `createExtensionUIContext().editor` 展示无 signal/timeout editor，再调用真实 `handleReloadCommand`；真实 `resetExtensionUI`/`cancelKind("extension")` 使 editor Promise 以 `undefined` 结算回调恰好一次，reload placeholder 随后实际 mount/focus/撤下且 reload Promise 完成，最终恢复 editor/idle；(b) 经真实 `createExtensionUIContext().select` 展示 extension select，并经真实 `showSelector` app factory 入队；通过真实 `subscribeToAgent` 捕获 connection callback 后发送 `session_replaced`，不得直接调用 reset：select 以 `undefined` 结算一次，app factory 只在 reset 后运行并保留/展示，随后真实 done 结算一次并恢复 editor/idle，同时 replacement 后续 state/render stubs 按序执行；(c) 在一个 visible + 一个 queued dialog 使 arbiter queue 非空时，经真实 extension context `custom(..., { overlay: true })` 与 `setFooter(factory)`，断言 overlay factory/`ui.showOverlay`/`onHandle` 和 footer factory/`footerSlot` 立即生效，不改 editor surface/focus、不改变 FIFO；overlay done/组件 dispose 与两个 dialog 后续结算保持各一次；(d) 用真实 extension context 创建 mounted 1 项 + queued 2 项并附每项 settlement observer，再在 harness 显式设 `isInitialized === true` 后调用真实 `teardownSessionUi`（必须进入真实 `stop`/`dialogArbiter.disposeAll`/`ui.stop`，而非直接 dispose）：在 `ui.stop` callback 进入时 mounted dispose 与三项 terminal outcome 已同步选定，queued factories 从未运行，surface clear/focus null/render 均已发生；Promise reaction 随后各执行一次。`ui.stop` 后触发保存的 late done 并重复 `teardownSessionUi`，不得再调用 surface/focus/render、再次 `ui.stop` 或产生第二次 settlement；断言 `ui.stop` options/order、`isInitialized` 转 false、`releasePromptStashSession`、terminal drain 与 theme watcher teardown boundary（系统边界 stub/spy，禁止真实终端副作用）。Async constructing 迟到组件 dispose 的独立边已由 group 6 caller evidence 与 Seam 1 覆盖，本 row 不把与 mounted-current 互斥的状态伪装进同一场景。

Suggested fixture level: compact - 跨全部迁移点的收口场景，需端到端 TUI 场景路径
Effective fixture level: expanded - source ownership oracle保护共享 `editorContainer` entrypoint，四条场景贯穿 real reload、connection `session_replaced`、extension overlay/footer 与 terminal teardown，命中 mandatory shared-state/cancellation trigger 和项目 TUI/session lifecycle domain triggers。Repair intensity: medium - 纯测试/fixture，无运行时代码改动；若测试暴露产品缺陷则必须回到实现 issue/修复流程，不得在本 PR 偷改。
Selected risk packs: Concurrency / shared state / ordering（kind-selective cancel、FIFO handoff、Promise reaction/terminal ordering）、Legacy compatibility / examples（overlay/footer 与既有 dialog values 保持）、Error handling / rollback / partial outputs（teardown 与 late edges）、TUI focus/render lifecycle（唯一 surface owner、excluded UI 即时生效）、Session/extension teardown lifecycle（session_replaced、reload reset、teardownSessionUi）。Public API/CLI、config、file IO/path、schema、auth/secrets、resource discovery、release/dependency、documentation migration 不选：本 PR 不改变运行时 API/配置/数据/依赖，源码读取仅为测试 oracle。
Invariant Matrix (group 10):
- Governing invariant: 所有非 overlay editor-surface dialog 只经共享 arbiter 改写 surface；reset 只取消 extension kind，overlay/footer 不入队，terminal teardown 在 UI stop 前同步选定全部请求终态并封死后续 UI。
- Source of truth: source negative-oracle owner whitelist；生产 `DialogArbiter` current/queue/disposed；真实 `InteractiveMode` reload/session event/extension context/teardown entrypoints。
- Regression rows: 越界 raw write -> owner+line 红；editor + reload -> cancel-once 后 placeholder 完整生命周期；select + queued app + session_replaced -> extension cancel/app survive；busy queue + overlay/footer -> immediate/nonblocking；mounted+queued teardown（`isInitialized=true`）-> settle-once、cleanup-before-stop、late done/重复 teardown no UI；async constructing 迟到组件边继承 group 6/Seam 1。
Boundary checklist (group 10): constructor/`setCustomEditorComponent`/arbiter injection owner ranges；extension editor/select/app/custom/footer callers；connection event queue；`teardownSessionUi` -> `stop` -> `disposeAll` -> `ui.stop`；unchanged group 1-9 sibling suites。
